...
# Rizz
# Final Spark shoots once. No breaks.
...
@export var spark_warning_duration_sec: float = 0.2
@export var spark_distance_from_boss: float = 90.0
@export var spark_speed: float = 1600.0
@export var spark_scale: float = 1.0
@export var spark_shoot_interval: float = 0.01
@export var spark_shoot_cone: float = 48.5
@export_group("Background Star Bullet Properties", "noise_")
@export var noise_type: PackedScene = STAR_BIG
@export var noise_count_inside: int = 0
@export var noise_count_outside: int = 33
@export var noise_speed: float = 500.0
@export var noise_interval_sec: float = 0.2
@export var noise_shoot_delay_sec: float = 0.7
@export var noise_targeted_cone: float = 112.0
...
var master_spark_has_been_shot: bool = false
...
func middle_attack() -> void:
	...
	setup_master_spark()
	leaf_gather_effect(1.3, 380.0, 350, 0.6, 3)
	play_sound(boss.long_charge_up)
	master_spark_target = get_static_point()
	stop_tracking = true
	await _set_timer(spark_warning_duration_sec + 1.4)
...
func _physics_process(delta: float) -> void:
	if stop_tracking and !FORCE_END_SPELLCARD:
		do_screen_shake(0.2, 15)
	if !set_target_before_shooting and !shoot_master_spark:
	...
	if shoot_master_spark:
		stop_tracking = false
		if !master_spark_has_been_shot:
			Audio.play_sound(boss.bullet_master_spark, boss)
			master_spark_has_been_shot = true
			await _set_timer(noise_shoot_delay_sec)
			if _boss_attack_interrupt(): return
			start_shooting_noise = true
		delete_warning_line()
		do_screen_shake(0.2, 35.0 * clampf(Engine.time_scale - 0.15, 0.1, 1.0), clampf(absf(1.0 - Engine.time_scale) / 10.0, 0.01, 0.1))
...
func move_boss_predefined() -> void:
	...
	var new_pos: Vector2 = anchor_pos - Vector2(0.0, 150.0)
	move_boss(new_pos, 0.5 + spark_warning_duration_sec)

func get_static_point() -> Vector2:
	return boss.boss_handler.global_position + Vector2(0.0, 240.0)
...
func setup_master_spark() -> void:
	...
	if !is_instance_valid(warning_line) and show_warning_line:
		...
		warning_line.target_position = get_static_point()
