...
@export var boss_name: String = "NULL"
...
@export var bullet_shoot_2: AudioStream
...
@export var summon_option: AudioStream
...
func _ready() -> void:
	...
	for i in children_list.size():
		if "bossgenerichandler" in children_list[i].name.to_lower():
