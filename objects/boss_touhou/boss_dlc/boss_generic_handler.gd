...
signal boss_clear
signal boss_damageless

@export_category("VS Touhou Boss")
@export_group("Trigger", "trigger_")
@export var trigger_boss: TouhouBossGeneric
@export var trigger_boss_borders: StaticBody2D
@export var trigger_spellcard_background: ParallaxBackground
@export var trigger_boss_HUD: GenericBossHUD
...
func boss_fight_logistics() -> void:
	...
	if is_instance_valid(trigger_boss_borders):
...
func _victory_sequence() -> void:
	...
	boss_clear.emit()
	if !player_was_hit: boss_damageless.emit()
